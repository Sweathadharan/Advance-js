...
const five=strings.filter(function(strings){
...
const concat = alphabets.reduce(function(res,alphabets){
  return res+alphabets
},'')
console.log(concat)
//Challenge 4: Given an array of numbers, write a code to return a first even number using find method.
const number = [1, 2, 3, 4, 5].find(function(number){
  return number%2==0;
});
console.log(number)
...
const ranks = [1, 5, 7, 8, 10, 7].findIndex(function(ranks){
  return ranks===7
});
console.log(ranks)
...
//2:6
//3:8
const num = [2, 4, 6, 8].forEach(function(item,index){
...
const marks = [35, 66, 25, 85, 75, 45].some(function(mark){
  return mark<45;
});
console.log(marks);
...
const positive = [2, 3, 4, -1].every(function(positive){
  return positive<0;
});
